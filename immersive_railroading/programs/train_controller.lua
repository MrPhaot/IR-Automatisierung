local DEFAULTS = {
  ...
  terminal_stop_capture_distance_m = 8.0,
...
local PROFILES = {
  conservative = {
    ...
    terminal_buffer_entry_speed_cap_mps = DEFAULTS.approach_stop_hold_speed_mps,
    terminal_buffer_final_speed_cap_mps = DEFAULTS.arrival_speed_mps,
    terminal_buffer_throttle_limit = DEFAULTS.approach_stop_throttle_limit,
    terminal_buffer_brake_window_m = 0,
    terminal_buffer_release_speed_mps = DEFAULTS.approach_stop_hold_speed_mps,
    terminal_success_buffer_tolerance_m = 1.0,
...
local function buffer_approach_target_speed(profile, physical_distance_minus_buffer_m)
  local soft_zone_m = profile.terminal_buffer_soft_zone_m or 0
  if soft_zone_m <= 0 or physical_distance_minus_buffer_m > soft_zone_m then
    return nil
  end

  local entry_speed_mps = profile.terminal_buffer_entry_speed_cap_mps or DEFAULTS.approach_stop_hold_speed_mps
  local final_speed_mps = profile.terminal_buffer_final_speed_cap_mps or entry_speed_mps
  local zone_ratio = clamp(physical_distance_minus_buffer_m / soft_zone_m, 0, 1)
...
local function terminal_buffer_required_stop_distance_m(speed_mps, brake_snapshot_mps2)
  ...
  local effective_brake_mps2 = math.max(brake_snapshot_mps2 or DEFAULTS.fallback_brake_mps2, DEFAULTS.min_brake_mps2)
  return (speed_mps * speed_mps) / (2 * effective_brake_mps2)
end

local function can_enter_stop_guidance(
  ...
  capture_distance_m = capture_distance_m or DEFAULTS.terminal_stop_capture_distance_m
  profile = profile or get_profile()

  if physical_distance_minus_buffer_m > capture_distance_m then
    return false, "outside_capture_window", 0
  end
  if physical_lateral_error_m > DEFAULTS.near_target_correction_lateral_m then
    return false, "lateral_error_too_large", 0
  end
  if route_alignment < DEFAULTS.terminal_stop_guidance_alignment_min
    and distance_to_physical_target_m > DEFAULTS.terminal_stop_guidance_distance_m then
    return false, "alignment_too_low", 0
  end
  ...
  local capture_speed_limit_mps = math.sqrt(
    2 * math.max(brake_snapshot_mps2 or DEFAULTS.fallback_brake_mps2, DEFAULTS.min_brake_mps2)
      * math.max(physical_distance_minus_buffer_m, 0)
  )
  capture_speed_limit_mps = math.min(
    capture_speed_limit_mps,
    profile.terminal_buffer_release_speed_mps or capture_speed_limit_mps
  )

  if required_stop_m > max_plausible_stop_m then
    return false, "implausible_terminal_energy", capture_speed_limit_mps
  end
  if physical_distance_minus_buffer_m < 0 then
    return true, "late_buffer_capture", capture_speed_limit_mps
  end
  if not stop_guidance_ready then
    return false, "insufficient_braking_room", capture_speed_limit_mps
  end
  if forward_speed_mps > capture_speed_limit_mps then
    return false, "capture_speed_too_high", capture_speed_limit_mps
  end
  return true, "buffer_window", capture_speed_limit_mps
end

local function physical_buffer_error_m(distance_to_physical_target_m, stop_buffer_m)
  return math.abs(distance_to_physical_target_m - math.max(stop_buffer_m or 0, 0))
end

local function is_terminal_success_physical_ok(profile, distance_to_physical_target_m, stop_buffer_m)
  local tolerance_m = (profile and profile.terminal_success_buffer_tolerance_m)
    or DEFAULTS.terminal_success_buffer_tolerance_m
  return physical_buffer_error_m(distance_to_physical_target_m, stop_buffer_m)
    <= tolerance_m
end

local function is_terminal_success_consistent(stop_ok, physical_ok)
  return stop_ok == true and physical_ok == true
end
...
local function should_enter_stop_guidance(
  ...
  if physical_distance_minus_buffer_m < 0 then
    return true, "late_buffer_capture"
  end
  return true, "buffer_window"
end
...
local function should_track_moving_away_sample(state, speed_toward_target_mps, reference_distance_m, guidance_mode, leg_mode)
  ...
  if state.progress_speed_mps > -DEFAULTS.moving_away_sample_progress_floor_mps then
    return false
  end
  if math.abs(speed_toward_target_mps) < DEFAULTS.moving_away_speed_gate_mps then
    return false
  end
  if (leg_mode == "pass_through" or guidance_mode == "route") and state.startup_guard_active then
    return reference_distance_m >= (state.initial_distance_to_target_m or reference_distance_m) + DEFAULTS.moving_away_distance_margin_m
      or speed_toward_target_mps <= -DEFAULTS.startup_guard_brake_speed_mps
  end
  return true
end
...
local function forward_buffer_settle_block_reason(
  profile,
  state,
  longitudinal_error_m,
  lateral_error_m,
  ...
  if (profile.buffer_settle_forward_speed_mps or 0) <= 0 then
  ...
  if longitudinal_error_m > (profile.buffer_settle_forward_max_longitudinal_m or 0) then
    return "beyond_forward_window"
  end
  ...
  if math.abs(speed_toward_target_mps) > (profile.buffer_settle_forward_speed_mps or 0) then
    return "speed_too_high"
  end
  if math.abs(axis_speed_mps) > math.max(profile.buffer_settle_forward_speed_mps or 0, DEFAULTS.arrival_speed_mps * 2) then
    return "axis_speed_too_high"
  end
  return nil
end
...
local function reverse_buffer_settle_block_reason(
  ...
  if raw_desired_reverser >= 0 or longitudinal_error_m >= 0 then
    return "target_ahead"
  end
  if math.abs(longitudinal_error_m) > (profile.buffer_settle_reverse_max_overshoot_m or 0) then
    return "beyond_reverse_window"
  end
  if lateral_error_m > (profile.buffer_settle_max_lateral_m or DEFAULTS.near_target_correction_lateral_m) then
    return "lateral_error_too_large"
  end
  ...
  if math.abs(speed_toward_target_mps) > (profile.buffer_settle_reverse_speed_mps or 0) then
    return "speed_too_high"
  end
  if math.abs(axis_speed_mps) > math.max(profile.buffer_settle_reverse_speed_mps or 0, DEFAULTS.arrival_speed_mps * 2) then
    return "axis_speed_too_high"
  end
  return nil
end
...
local function can_enforce_moving_away_brake(state, speed_toward_target_mps, reference_distance_m, guidance_mode, leg_mode)
  if not should_force_moving_away_brake(state, speed_toward_target_mps) then
    return false
  end
  if leg_mode == "pass_through" or guidance_mode == "route" then
    return reference_distance_m >= (state.initial_distance_to_target_m or reference_distance_m) + DEFAULTS.moving_away_distance_margin_m
      or speed_toward_target_mps <= -DEFAULTS.startup_guard_brake_speed_mps
  end
  return true
...
local function select_motion_mode(state, speed_toward_target_mps, target_speed_mps, distance_to_target_m, stop_context, profile, moving_away_brake_allowed)
  ...
  if state.buffer_settle_mode and state.buffer_settle_mode ~= "none" and not must_stop_now then
    return "drive"
  end
  ...
  if state.near_target_correction_active then
    ...
    if moving_away_brake_allowed then
      return "brake"
    end
  end
...
local function make_speed_plan(speed_limit_mps, speed_command_mps, desired_reverser, reason, force_mode)
  local limit_mps = math.max(speed_limit_mps or 0, 0)
  local command_mps = math.max(speed_command_mps or limit_mps, 0)
  command_mps = math.min(command_mps, limit_mps)
  return {
    speed_limit_mps = limit_mps,
    speed_command_mps = command_mps,
    -- Backward-compatible alias while downstream tools/log parsers migrate.
    v_target_mps = command_mps,
...
local function update_terminal_speed_command(previous_cmd_mps, speed_limit_mps, speed_toward_target_mps, committed_stop)
  speed_limit_mps = math.max(speed_limit_mps or 0, 0)
  if not committed_stop then
    return speed_limit_mps
  end

  local current_speed_mps = math.max(speed_toward_target_mps or 0, 0)
  local seeded_command_mps = math.min(current_speed_mps, speed_limit_mps)
  if previous_cmd_mps == nil then
    return seeded_command_mps
  end
  return math.min(math.max(previous_cmd_mps, 0), speed_limit_mps)
end

local function compute_longitudinal_effort(
  pid,
  integral,
  previous_error,
  speed_error,
  ...
  derivative_gain_scale = derivative_gain_scale == nil and 1 or derivative_gain_scale

  local next_integral = clamp(integral + speed_error * dt_s, -integral_limit, integral_limit)
  local prev_actual_speed_mps = previous_actual_speed_mps
  if prev_actual_speed_mps == nil then
    prev_actual_speed_mps = actual_speed_mps
  end
  local d_meas = -((actual_speed_mps or 0) - (prev_actual_speed_mps or 0)) / dt_s
  local raw_effort = pid.kp * speed_error + pid.ki * next_integral + pid.kd * d_meas * derivative_gain_scale
  ...
  return effort, next_integral, speed_error, d_meas
end
...
local function begin_leg(runtime_context)
  ...
  return {
    ...
    buffer_settle_eligible = false,
    buffer_settle_block_reason = "inactive",
...
local function pass_through_handoff_reason(distance_to_target_m, previous_position, position, leg)
  local crossed_plane = waypoint_plane_crossed(previous_position, position, leg)
  if leg.next_leg_mode == "terminal" and distance_to_target_m <= DEFAULTS.route_leg_handoff_m and not crossed_plane then
    if distance_to_target_m <= (DEFAULTS.route_leg_handoff_m * 0.5) then
      return "within_terminal_handoff_distance"
    end
    return nil
  end
...
local function control_loop(remote, target, requested_cruise_kmh, stop_buffer_m, profile_name, logger)
  ...
  while true do
    ...
    local moving_away_sample = should_track_moving_away_sample(
      state,
      speed_toward_target_mps,
      distance_to_target_m,
      "route",
    ...
    local moving_away_brake_allowed = can_enforce_moving_away_brake(
      state,
      speed_toward_target_mps,
      distance_to_target_m,
      "route",
...
local function build_route_plan(route_name, waypoints, cruise_kmh, stop_buffer_m, profile_name)
  ...
  for index, waypoint in ipairs(waypoints) do
    ...
    legs[index] = {
      ...
      next_leg_mode = index + 1 < #waypoints and "pass_through" or (index + 1 == #waypoints and "terminal" or nil),
...
local function run_route_leg(remote, route_plan, leg, runtime_context, leg_transition_reason, logger)
  ...
  while true do
    ...
    local to_stop_target = to_physical_target
    local distance_to_stop_target_m = distance_to_physical_target_m
    local physical_longitudinal_error_m = 0
    local physical_lateral_error_m = 0
    local physical_longitudinal_distance_m = 0
    local stop_longitudinal_error_m = 0
    local stop_lateral_error_m = 0
    local stop_longitudinal_distance_m = 0
    local raw_progress_speed_mps = 0
    ...
    local terminal_buffer_reserve_m = leg.mode == "terminal"
      and (leg.effective_stop_buffer_m or math.max(route_plan.stop_buffer_m or 0, 0))
      or 0
    local physical_distance_minus_buffer_m = distance_to_physical_target_m - terminal_buffer_reserve_m
    local physical_buffer_error_m_value = physical_buffer_error_m(distance_to_physical_target_m, terminal_buffer_reserve_m)
    ...
    if fresh_target_axis and (leg.mode ~= "terminal" or state.guidance_mode ~= "stop") then
      -- The terminal leg still follows the real route geometry until the late
      -- stop capture freezes a final halting frame near the end of the run.
      state.target_line_axis = blend_axes(state.target_line_axis, fresh_target_axis, 0.1)
    end

    local physical_reference_axis = state.target_line_axis
    ...
    local route_alignment = velocity_axis and abs_dot(velocity_axis, physical_reference_axis) or 0
    ...
    local physical_axis_speed_mps = vector_dot(runtime_context.filtered_velocity, physical_reference_axis)
    local physical_speed_toward_target_mps = physical_axis_speed_mps * (physical_longitudinal_error_m >= 0 and 1 or -1)
    ...
    if leg.mode == "terminal" then
      ...
      state.terminal_success_physical_ok = is_terminal_success_physical_ok(
        profile,
        distance_to_physical_target_m,
        terminal_buffer_reserve_m
      )
      state.terminal_success_consistent = false
      local provisional_stop_target = vector_sub(
        physical_target,
        vector_scale(physical_reference_axis, terminal_buffer_reserve_m)
      )
      local provisional_to_stop_target = vector_sub(provisional_stop_target, position)
      stop_longitudinal_error_m = vector_dot(provisional_to_stop_target, physical_reference_axis)
      stop_lateral_error_m = vector_length(vector_reject(provisional_to_stop_target, physical_reference_axis))
      ...
      if state.guidance_mode ~= "stop" then
        ...
        state.terminal_stop_target = physical_target
        state.effective_stop_buffer_m = terminal_buffer_reserve_m
        state.late_stop_capture = false

        local use_stop_guidance, stop_guidance_entry_reason, capture_speed_limit_mps = can_enter_stop_guidance(
          distance_to_physical_target_m,
          physical_distance_minus_buffer_m,
          physical_lateral_error_m,
          route_alignment,
          profile.terminal_buffer_capture_distance_m,
          physical_speed_toward_target_mps,
          state.terminal_brake_snapshot_mps2,
          profile
        )
        state.stop_guidance_ready = use_stop_guidance
        state.stop_guidance_block_reason = use_stop_guidance and "ready" or stop_guidance_entry_reason
        state.terminal_stop_capture_speed_limit_mps = capture_speed_limit_mps or 0
        if use_stop_guidance then
          state.terminal_stop_axis = physical_reference_axis
          state.terminal_route_axis = state.terminal_stop_axis
          ...
          state.guidance_mode = "stop"
          state.progress_speed_mps = 0
          ...
          runtime_context.previous_distance_to_target_m = distance_to_physical_target_m
          state.stop_guidance_entry = true
          state.stop_guidance_entry_reason = stop_guidance_entry_reason
          state.late_stop_capture = stop_longitudinal_error_m < -DEFAULTS.arrival_longitudinal_m
          if state.late_stop_capture then
          ...
          emit_line(logger, ("stop_guidance_entry route_name=%s leg=%d/%d reason=%s physical_distance=%.2fm physical_distance_minus_buffer=%.2fm physical_longitudinal=%.2fm physical_lateral=%.2fm stop_longitudinal=%.2fm stop_buffer_m=%.2fm late_stop_capture=%s physical_target=%s terminal_stop_target=%s"):format(
            route_plan.name,
            leg.index,
            leg.count,
            ...
            tostring(state.late_stop_capture),
            format_point(physical_target),
            format_point(state.terminal_stop_target or physical_target)
          ))
        end
      else
        state.terminal_route_axis = state.terminal_stop_axis or state.terminal_route_axis
        if state.terminal_stop_axis then
          state.terminal_stop_target, state.effective_stop_buffer_m, state.terminal_stop_axis =
            compute_terminal_stop_geometry(
              physical_target,
              state.terminal_stop_axis,
      ...
      if state.guidance_mode == "stop" and state.terminal_stop_axis then
        ...
        state.moving_away_reference = "physical"
      end
    else
      ...
      state.terminal_speed_commit_active = false
      state.terminal_stop_capture_speed_limit_mps = 0
      state.terminal_entry_alignment = 0
      state.terminal_success_stop_ok = false
      state.terminal_success_physical_ok = false
      state.terminal_success_consistent = false
      state.physical_buffer_error = 0
      state.buffer_settle_mode = "none"
      state.buffer_settle_eligible = false
      state.buffer_settle_block_reason = "non_terminal_leg"
    end
    ...
    local raw_desired_reverser = longitudinal_error_m >= 0 and 1 or -1
    local desired_reverser = raw_desired_reverser
    if leg.mode == "terminal" and state.guidance_mode == "stop" and distance_to_target_m <= terminal_stop_zone_m then
      desired_reverser = state.active_reverser
    end
    ...
    if state.guidance_mode == "stop" then
      ...
      state.stop_previous_distance_to_target_m = nil
      state.stop_distance_delta_m = 0
      state.stop_progress_speed_mps = 0
      state.stop_progress_initialized = false
    end
    state.startup_guard_active = ((now - state.started_at) <= DEFAULTS.startup_guard_duration_s)
      and distance_to_target_m <= state.initial_distance_to_target_m + DEFAULTS.startup_guard_distance_margin_m
    state.curve_guard_active = state.axis_alignment < DEFAULTS.curve_guard_alignment
    ...
    local terminal_brake_snapshot_mps2 = state.terminal_brake_snapshot_mps2 or brake_model.full_service_mps2
    local stop_cap_brake_model = leg.mode == "terminal"
      and {full_service_mps2 = terminal_brake_snapshot_mps2}
      or brake_model
    local pid = nil
    local target_speed_mps = leg.mode == "terminal"
      and stop_speed_cap(
        state.guidance_mode == "stop" and distance_to_stop_target_m or distance_to_physical_target_m,
        state.guidance_mode == "stop" and 0 or terminal_buffer_reserve_m,
        stop_cap_brake_model,
        characteristics.cruise_mps,
        profile
      )
    ...
    -- Apply travel speed scale for pass_through legs only
    if leg.mode == "pass_through" then
      target_speed_mps = math.min(
        target_speed_mps * (profile.travel_speed_scale or 1.0),
        characteristics.cruise_mps * (profile.travel_speed_scale or 1.0)
      )
    end

    if leg.mode == "terminal" and state.guidance_mode == "route" and profile.name == "fast" then
      local brake_window_m = profile.terminal_buffer_brake_window_m or 0
      local release_speed_mps = profile.terminal_buffer_release_speed_mps or 0
      if brake_window_m > 0 and physical_distance_minus_buffer_m <= brake_window_m then
        if physical_speed_toward_target_mps > math.max(target_speed_mps, release_speed_mps) then
          terminal_buffer_brake_active = true
          terminal_buffer_brake_reason = "speed_above_buffer_target"
        else
          terminal_buffer_brake_reason = "speed_within_buffer_target"
        end
      else
        terminal_buffer_brake_reason = "outside_brake_window"
      end
    end

    if leg.mode == "terminal"
      and state.guidance_mode == "stop"
      and distance_to_stop_target_m <= terminal_stop_zone_m
      and stop_lateral_error_m <= DEFAULTS.arrival_lateral_m then
      target_speed_mps = 0
    end
    ...
    local terminal_success_stop_ok = false
    local terminal_success_physical_ok = false
    local terminal_success_consistent = false
    ...
    local buffer_settle_mode = "none"
    local buffer_settle_eligible = false
    local buffer_settle_block_reason = leg.mode == "terminal" and "route_guidance_active" or "non_terminal_leg"
    local buffer_settle_active = false
    local buffer_settle_reason = "inactive"
    local buffer_success_tolerance_m = profile.terminal_success_buffer_tolerance_m or DEFAULTS.terminal_success_buffer_tolerance_m

    if leg.mode == "terminal" and state.guidance_mode == "stop" then
      ...
      stop_context = {
        required_stop_m = required_stop_m,
        in_approach_stop = distance_to_stop_target_m <= math.max(
          DEFAULTS.approach_stop_distance_m,
          required_stop_m + DEFAULTS.approach_stop_margin_m
        ),
        in_no_reverse_approach = distance_to_stop_target_m <= math.max(
          profile.no_reverse_distance_m,
          required_stop_m + profile.required_stop_margin_m
        ),
        must_stop_now = speed_toward_target_mps > DEFAULTS.arrival_speed_mps
          and required_stop_m + profile.required_stop_margin_m >= distance_to_stop_target_m,
      }
      if stop_context.in_no_reverse_approach then
        target_speed_mps = math.min(target_speed_mps, characteristics.cruise_mps * profile.approach_stop_target_speed_scale)
      end
      ...
      local target_ahead_stalled = stop_context.in_no_reverse_approach
        ...
        and math.abs(state.stop_progress_speed_mps) <= DEFAULTS.terminal_deadlock_stall_speed_mps
      ...
      if state.near_target_correction_active then
        target_speed_mps = math.min(target_speed_mps, DEFAULTS.near_target_correction_speed_mps)
        if raw_desired_reverser == state.active_reverser
          and distance_to_stop_target_m > DEFAULTS.overshoot_recovery_distance_m then
          state.near_target_correction_active = false
          state.near_target_resolution = "idle"
        end
      end

      terminal_success_stop_ok = is_strict_arrival(
        distance_to_stop_target_m,
        stop_longitudinal_distance_m,
        stop_lateral_error_m,
        speed_toward_target_mps
      ) or near_target_hold or is_terminal_limit_arrival(
        distance_to_stop_target_m,
        stop_longitudinal_distance_m,
        stop_lateral_error_m,
        speed_toward_target_mps,
      ...
      state.terminal_success_stop_ok = terminal_success_stop_ok
      state.terminal_success_physical_ok = terminal_success_physical_ok
      state.terminal_success_consistent = terminal_success_consistent

      if state.stop_first_active or state.near_target_correction_active then
        buffer_settle_block_reason = "higher_priority_terminal_recovery"
        terminal_recovery_block = "higher_priority_terminal_recovery"
      elseif terminal_success_consistent then
        buffer_settle_block_reason = "already_successful"
        terminal_recovery_block = "already_successful"
      else
        local forward_block = forward_buffer_settle_block_reason(
          profile,
          state,
          stop_longitudinal_error_m,
          stop_lateral_error_m,
      ...
      if buffer_settle_mode == "none" then
        terminal_recovery_eligible = false
      elseif buffer_settle_mode == "forward" then
        terminal_recovery_eligible = false
        terminal_recovery_block = "forward_settle_active"
      else
        terminal_recovery_eligible = true
        terminal_recovery_block = "eligible"
      end
    end

    if leg.mode == "terminal" and state.guidance_mode == "stop" and buffer_settle_mode ~= "none" then
      speed_toward_target_mps = motion_axis_speed_mps * desired_reverser
    end
    ...
    local speed_command_mps = update_terminal_speed_command(
    ...
    if committed_stop then
      state.terminal_speed_commit_active = true
      state.terminal_speed_command_mps = speed_command_mps
      if not state.terminal_pid then
        state.terminal_pid = derive_pid(characteristics, {
          full_service_mps2 = state.terminal_brake_snapshot_mps2 or brake_model.full_service_mps2,
        })
      end
    ...
    if leg.mode == "terminal" then
      if state.guidance_mode == "stop" then
        ...
        hold = terminal_success_consistent and (
          is_strict_arrival(
            distance_to_stop_target_m,
            stop_longitudinal_distance_m,
            stop_lateral_error_m,
        ...
        terminal_limit_hold = terminal_success_consistent and is_terminal_limit_arrival(
          distance_to_stop_target_m,
          stop_longitudinal_distance_m,
          stop_lateral_error_m,
          speed_toward_target_mps,
      ...
      state.buffer_settle_mode = buffer_settle_mode
      state.buffer_settle_eligible = buffer_settle_eligible
      ...
      state.buffer_settle_mode = "none"
      state.buffer_settle_eligible = false
      state.buffer_settle_block_reason = "pass_through_leg"
      state.terminal_success_stop_ok = false
      state.terminal_success_physical_ok = false
      state.terminal_success_consistent = false
    ...
    local speed_plan = make_speed_plan(speed_limit_mps, speed_command_mps, desired_reverser, "speed_tracking", "auto")
    local effort_cmd = 0
    ...
    if hold then
      ...
      speed_plan = make_speed_plan(speed_limit_mps, speed_command_mps, desired_reverser, terminal_limit_hold and "arrived_within_v1_limit"
      ...
      if now - runtime_context.settled_since >= DEFAULTS.settle_time_s then
        ...
        local completion_reason = terminal_limit_hold and "arrived_within_v1_limit" or "arrived_at_target"
        emit_line(logger, ("%s route_name=%s leg=%d/%d learned_brake=%.3f m/s^2 samples=%d distance=%.2fm physical_distance=%.2fm longitudinal=%.2fm lateral=%.2fm stop_buffer_m=%.2fm physical_buffer_error=%.2fm terminal_success_stop_ok=%s terminal_success_physical_ok=%s terminal_success_consistent=%s physical_target=%s terminal_stop_target=%s"):format(
          completion_reason,
          route_plan.name,
          leg.index,
          leg.count,
          ...
          stop_lateral_error_m,
          state.effective_stop_buffer_m,
          state.physical_buffer_error or 0,
      ...
      if leg.mode == "terminal" and state.guidance_mode == "stop" and state.near_target_resolution == "limit" then
        ...
        state.final_forward_crawl = false
        speed_plan = make_speed_plan(speed_limit_mps, 0, state.active_reverser, "near_target_limit", "hold")
        state.mode = "hold"
        state.phase = "hold"
        state.reason = speed_plan.reason
        control = {
    ...
    local low_speed_terminal_brake_learning_block = leg.mode == "terminal"
      and (state.guidance_mode == "stop" or stop_context.in_no_reverse_approach)
      and math.abs(speed_toward_target_mps) <= 1.0
    if runtime_context.previous_time
      and runtime_context.previous_speed_toward_target_mps > 0
      and not low_speed_terminal_brake_learning_block then
    ...
    if now - runtime_context.last_report >= DEFAULTS.report_interval_s then
      ...
      emit_line(logger, (
        ...
        state.distance_delta_m,
        state.progress_speed_mps,
        state.stop_distance_delta_m or 0,
        state.stop_progress_speed_mps or 0,
        tostring(state.stop_progress_initialized),
        state.moving_away_confidence,
        tostring(state.startup_guard_active),
        tostring(state.curve_guard_active),
        required_stop_m,
        state.effective_stop_buffer_m,
        state.terminal_brake_snapshot_mps2 or 0,
        state.terminal_stop_capture_speed_limit_mps or 0,
        buffer_target_speed_mps,
        buffer_speed_cap_active_mps,
        buffer_throttle_limit_active,
        tostring(terminal_buffer_brake_active),
        terminal_buffer_brake_reason,
        tostring(buffer_settle_active),
        state.buffer_settle_mode,
        tostring(state.buffer_settle_eligible),
        state.buffer_settle_block_reason,
        buffer_settle_reason,
